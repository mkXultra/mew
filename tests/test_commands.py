...
class CommandTests(unittest.TestCase):
    ...
    def test_task_propose_next_human_output_for_explicit_candidate(self):
        ...
        self.assertIn("lane_dispatch:", output)
        self.assertIn("authoritative_lane", output)
    ...
    def test_task_propose_next_explicit_blocked_candidate_records_even_with_later_safe_candidate(self):
        ...
        self.assertTrue(proposal["blocked"])
        self.assertTrue(proposal["governance_violation"])
        self.assertIn("repair_route", proposal["lane_dispatch"])
        self.assertIn("M6.14 repair episode", proposal["lane_dispatch"]["repair_route"])
    ...
    def test_task_propose_next_missing_candidate_keeps_lane_dispatch_schema(self):
        old_cwd = os.getcwd()
        with tempfile.TemporaryDirectory() as tmp:
            os.chdir(tmp)
            try:
        ...
        proposal = json.loads(stdout.getvalue())
        self.assertTrue(proposal["blocked"])
        self.assertEqual(proposal["blocked_reason"], "candidate task not found: 999")
        self.assertEqual(proposal["lane_dispatch"]["fallback_lane"], "tiny")
        self.assertIn("M6.14 repair episode", proposal["lane_dispatch"]["repair_route"])
        records = state.get("selector_proposals") or []
        self.assertEqual(len(records), 1)
        self.assertEqual(records[0]["proposal"]["lane_dispatch"], proposal["lane_dispatch"])
    ...
    def test_task_propose_next_record_persists_proposed_selector_record(self):
        ...
        self.assertEqual(record["previous_task_id"], previous["id"])
        self.assertEqual(record["proposed_task_id"], candidate["id"])
        self.assertEqual(record["proposal"], proposal)
    ...
    def test_task_propose_next_record_persists_blocked_selector_record(self):
        ...
        self.assertIn("ROADMAP_STATUS.md", record["proposal"]["blocked_reason"])
        self.assertIn("repair_route", record["proposal"]["lane_dispatch"])
    ...
    def test_task_propose_next_record_only_mutates_selector_records(self):
        ...
        self.assertEqual(code, 0)
        records = state_after.get("selector_proposals") or []
        self.assertEqual(len(records), 1)
    ...
    def test_task_execute_proposal_cli_approved_proposal_records_handoff_without_dispatch(self):
        old_cwd = os.getcwd()
        with tempfile.TemporaryDirectory() as tmp:
            os.chdir(tmp)
            try:
                ...
                proposal = commands_module.load_state()["selector_proposals"][0]
                with redirect_stdout(StringIO()):
                    self.assertEqual(
                        main(["task", "approve-proposal", str(proposal["id"]), "--reason", "ship it", "--json"]),
                        0,
                    )
                state_before = commands_module.load_state()
                proposal = state_before["selector_proposals"][0]
        ...
        expected_next_command = f"./mew work {candidate['id']} --start-session"
        self.assertEqual(code, 0)
        attempt = json.loads(stdout.getvalue())
        self.assertEqual(attempt["proposal_id"], proposal["id"])
        self.assertEqual(attempt["proposed_task_id"], candidate["id"])
        self.assertEqual(attempt["status"], "handoff_ready")
        self.assertEqual(attempt["approval_status"], "approved")
        self.assertEqual(attempt["reviewer_decision"], "approved")
        self.assertEqual(attempt["reviewer_reason"], "ship it")
        self.assertTrue(attempt["reviewed_at"])
        self.assertFalse(attempt["auto_run"])
        self.assertEqual(attempt["next_command"], expected_next_command)
        self.assertTrue(attempt["timestamp"])
        self.assertIn("supervised handoff ready", stderr.getvalue())
        self.assertIn(expected_next_command, stderr.getvalue())
        self.assertEqual(state_after["selector_execution_attempts"], [attempt])
        self.assertEqual(state_after.get("agent_runs") or [], [])
        self.assertEqual([task["status"] for task in state_after["tasks"]], ["ready", "ready"])
        state_without_attempts = dict(state_after)
    ...
    def test_workbench_summarizes_task_resume_surface(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertIn("resume_next_action: refresh working memory from latest edit_file result", output)
                self.assertIn("then continue the work session", output)
