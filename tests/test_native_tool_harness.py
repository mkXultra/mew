...
def test_native_harness_runs_finish_time_final_verifier_closeout_after_latest_source_mutation(tmp_path: Path) -> None:
    ...
    assert result.metrics["completion_resolver_decision_count"] == 0
    assert result.metrics["native_finish_gate_latest_decision"]["lane_status"] == "completed"
    assert result.metrics["native_finish_gate_latest_decision"]["closeout_refs"]
    ...
    assert closeout_args["command_intent"] == "finish_verifier"
    assert closeout_args["execution_contract"]["stage"] == "verification"
    closeout_output = next(
        item
        for item in result.transcript.items
...
def test_native_harness_trusted_closeout_exit_zero_overrides_typed_evidence_blockers(tmp_path: Path) -> None:
    ...
    assert decision["closeout_refs"]
    finish_output = next(item for item in result.transcript.items if item.kind == "finish_output")
    assert "invalid_typed_evidence_ref" not in finish_output.output_text_or_ref
    assert validate_native_transcript_pairing(result.transcript).valid is True
...
def test_native_harness_finish_verifier_planner_request_includes_external_failure(
    tmp_path: Path,
) -> None:
    class PlanningProvider(NativeFakeProvider):
        planner_requests: list[dict[str, object]]
    ...
    result = run_native_implement_v2(
        _lane_input(
            ...
            finish_verifier_external_failure={
                "failed_tests": ["test_vm_execution"],
                "expected_stdout_substrings": ["I_InitGraphics: DOOM screen size: w x h: 320 x 200"],
                "sampling_semantics": {"waits_for_artifact_path": True},
            },
        ),
        provider=provider,
        max_turns=1,
    ...
    assert result.status == "blocked"
    request = provider.planner_requests[0]
    assert request["external_verifier_failure"]["failed_tests"] == ["test_vm_execution"]  # type: ignore[index]
    assert "I_InitGraphics" in json.dumps(request["external_verifier_failure"])  # type: ignore[index]
    decision = result.metrics["finish_verifier_planner_latest_decision"]
    assert decision["fallback_source"] == ""
    assert decision["reject_blockers"] == ["finish_verifier_external_failure_shape_missing"]


def test_finish_verifier_planner_rejects_plain_run_when_external_failure_present() -> None:
    class Provider:
        def plan_finish_verifier_command(self, request: dict[str, object]) -> dict[str, object]:
            return {"command": "node vm.js", "cwd": ".", "confidence": "high"}

    result = run_finish_verifier_planner_loop(
        FinishVerifierPlannerLoopRequest(
            lane_attempt_id="lane",
            turn_id="turn",
            finish_call_id="finish",
            task_id="task",
            task_description="Run node vm.js and ensure stdout plus frame are externally visible.",
            task_contract={},
            latest_mutation={},
            recent_tool_results=(),
            candidate_paths=(),
            ...
            policy=FinishVerifierPlannerLoopPolicy(enabled=True),
        ),
        planner_provider=Provider(),
    )

    assert result.status == "rejected"
    assert result.blockers == ("finish_verifier_external_failure_shape_missing",)


def test_finish_verifier_planner_accepts_observable_assertion_when_external_failure_present() -> None:
    class Provider:
        def plan_finish_verifier_command(self, request: dict[str, object]) -> dict[str, object]:
            return {
                "command": "test -s /tmp/frame.bmp && grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt",
                "cwd": ".",
                "confidence": "medium",
            }
    ...
    assert result.plan.command.startswith("test -s /tmp/frame.bmp")


@pytest.mark.parametrize(
    "command",
    [
        "true /tmp/frame.bmp 'I_InitGraphics: DOOM screen size: w x h: 320 x 200'",
        "node vm.js --frame /tmp/frame.bmp --expected 'I_InitGraphics: DOOM screen size: w x h: 320 x 200'",
        "test -s /tmp/frame.bmp # I_InitGraphics: DOOM screen size: w x h: 320 x 200",
        "test -s /tmp/frame.bmp && grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /app/tests/test_outputs.py",
        "test -s /tmp/frame.bmp && grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /app/tests/test_outputs.py && test -f vm.js",
        "test -s /tmp/frame.bmp && grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' vm.js",
        "test -s /tmp/frame.bmp && grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt vm.js",
        "test -s /tmp/frame.bmp && grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt /app/tests/test_outputs.py",
        "grep -q --label=/tmp/frame.bmp 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt",
        "test /tmp/frame.bmp = /tmp/frame.bmp && grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt",
        "file /tmp/frame.bmp && grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt",
        "test -s /tmp/other -o /tmp/frame.bmp = /tmp/frame.bmp && grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt",
        "awk 'BEGIN { print \"I_InitGraphics: DOOM screen size: w x h: 320 x 200\" }' /tmp/vm_stdout.txt && test -s /tmp/frame.bmp",
        "test ! -s /tmp/frame.bmp && grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt",
        "test -s /tmp/frame.bmp -o 1 = 1 && grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt",
        "test -s /tmp/frame.bmp && grep -vq 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt",
        "test -s /tmp/frame.bmp && grep -L 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt",
        "test -s /tmp/frame.bmp && grep --files-without-match 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt",
        "test -s /tmp/frame.bmp && rg -v 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt",
        "test -s /tmp/frame.bmp && rg --files-without-match 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt",
        "test -s $(echo /tmp/frame.bmp) && grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt",
        "test -s `/bin/echo /tmp/frame.bmp` && grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt",
        "node vm.js",
        "grep -q totally-unrelated README.md",
        "grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/frame.bmp.bak",
        "grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /app/tests/test_outputs.py",
        "stat /tmp/other-frame.bmp",
        "test -f vm.js",
    ],
)
def test_finish_verifier_planner_rejects_commands_that_do_not_match_external_failure_shape(
    command: str,
) -> None:
    class Provider:
        def plan_finish_verifier_command(self, request: dict[str, object]) -> dict[str, object]:
            return {"command": command, "cwd": ".", "confidence": "medium"}
...
@pytest.mark.parametrize(
    "command",
    [
        "test -s /tmp/frame.bmp && grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt || true",
        "test -s /tmp/frame.bmp && grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt; true",
        "test -s /tmp/frame.bmp && grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt | cat",
        "test -s /tmp/frame.bmp && grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt > /tmp/out",
    ],
)
def test_finish_verifier_planner_rejects_masked_external_failure_assertions(
    command: str,
) -> None:
    class Provider:
        def plan_finish_verifier_command(self, request: dict[str, object]) -> dict[str, object]:
...
def test_finish_verifier_planner_rejects_missing_multi_artifact_external_failure_assertion() -> None:
    class Provider:
        def plan_finish_verifier_command(self, request: dict[str, object]) -> dict[str, object]:
            return {
                "command": "test -s /tmp/frame.bmp && grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/vm_stdout.txt",
    ...
    result = run_finish_verifier_planner_loop(
        FinishVerifierPlannerLoopRequest(
            ...
            external_verifier_failure={
                "expected_stdout_substrings": ["I_InitGraphics: DOOM screen size: w x h: 320 x 200"],
                "artifact_paths": ["/tmp/frame.bmp", "/tmp/audio.raw"],
            },
            policy=FinishVerifierPlannerLoopPolicy(enabled=True),
        ),
        planner_provider=Provider(),
...
def test_finish_verifier_planner_rejects_prior_verifier_log_as_stdout_target() -> None:
    class Provider:
        def plan_finish_verifier_command(self, request: dict[str, object]) -> dict[str, object]:
            return {
                "command": "test -s /tmp/frame.bmp && grep -q 'I_InitGraphics: DOOM screen size: w x h: 320 x 200' /tmp/old-run/verifier/test-stdout.txt",
                "cwd": ".",
                "confidence": "medium",
            }
    ...
    result = run_finish_verifier_planner_loop(
        FinishVerifierPlannerLoopRequest(
            ...
            candidate_paths=(),
            external_verifier_failure={
                "source_path": "/tmp/old-run/verifier/test-stdout.txt",
                "expected_stdout_substrings": ["I_InitGraphics: DOOM screen size: w x h: 320 x 200"],
                "artifact_paths": ["/tmp/frame.bmp"],
            },
            policy=FinishVerifierPlannerLoopPolicy(enabled=True),
...
def test_finish_verifier_planner_rejects_subject_only_when_external_failure_has_no_concrete_shape() -> None:
    class Provider:
        def plan_finish_verifier_command(self, request: dict[str, object]) -> dict[str, object]:
            return {"command": "node vm.js", "cwd": ".", "confidence": "medium"}
    ...
    result = run_finish_verifier_planner_loop(
        FinishVerifierPlannerLoopRequest(
            ...
            recent_tool_results=(),
            candidate_paths=(),
            external_verifier_failure={"failed_tests": ["test_vm_execution"]},
            policy=FinishVerifierPlannerLoopPolicy(enabled=True),
        ),
        planner_provider=Provider(),
    )
...
def test_native_harness_semantic_finish_time_final_verifier_exit_zero_is_external_oracle_risk(tmp_path: Path) -> None:
    provider = NativeFakeProvider.from_item_batches(
        [
            [
                fake_call(
...
def test_native_closeout_resolves_only_verifier_typed_gate_missing_obligations() -> None:
    ...
    assert not _finish_gate_block_resolved_by_closeout(
        ...
        gate={
            "blockers": [{"code": "runtime_final_verifier_artifact_evidence"}],
            "missing_obligations": [{"id": "oracle:source:vm-js", "kind": "source_grounding"}],
        },
        closeout_context=closeout,
    )
    assert _finish_gate_block_resolved_by_closeout(
...
def test_native_completion_resolver_accepts_final_verifier_runtime_artifact_closeout(tmp_path: Path) -> None:
    finish_call = NativeTranscriptItem(
        ...
        call_id="finish-call",
        tool_name="finish",
        arguments_json_text=json.dumps({"outcome": "completed", "summary": "verified"}),
    )
    ...
    closeout = _NativeCloseoutContext(
        closeout_refs=("implement-v2-exec://attempt/command-final-verifier/terminal",),
    ...
    assert resolver_input.closeout_refs == ("implement-v2-exec://attempt/command-final-verifier/terminal",)
    assert resolver_input.fresh_verifier_refs == ("implement-v2-exec://attempt/command-final-verifier/terminal",)
...
def test_native_harness_final_verifier_closeout_source_mutation_blocks_native_gate(tmp_path: Path) -> None:
    provider = NativeFakeProvider.from_item_batches(
        [
            [
                fake_call(
    ...
    result = run_native_implement_v2(
        _lane_input(
            tmp_path,
            allow_verify=True,
            verify_command="python -c 'from pathlib import Path; Path(\"vm.js\").write_text(\"mutated\\\\n\")'",
            final_verifier_closeout_seconds=3,
        ),
        provider=provider,
        max_turns=1,
