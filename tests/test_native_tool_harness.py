...
def test_live_native_provider_retries_transient_websocket_close_same_turn(tmp_path: Path) -> None:
    lane_input = _lane_input(tmp_path)
    provider = NativeCodexResponsesProvider(
        lane_input=lane_input,
        auth={"access_token": "x"},
    ...
    with patch(
        "mew.implement_lane.native_tool_harness.call_codex_native_responses_websocket",
        side_effect=[
            codex_api.CodexTransientTransportError(
                "websocket closed while reading frame"
            ),
            completed,
        ],
    ) as call:
        result = provider.next_response(
            {
                "lane_attempt_id": "ws-native:task-native:implement_v2:native",
                "turn_index": 1,
                "input_items": [
    ...
    attempts = provider.requests[0]["transport_attempts"]
    assert [attempt["status"] for attempt in attempts] == [
        "retryable_error",
        "completed",
    ]
    assert provider.requests[0]["native_transport_recovered"] is True
    assert provider.requests[0]["retryable"] is False


def test_live_native_provider_retry_preserves_previous_response_delta(tmp_path: Path) -> None:
    lane_input = _lane_input(tmp_path)
    provider = NativeCodexResponsesProvider(
        lane_input=lane_input,
        auth={"access_token": "x"},
    ...
    assert second_request["input"] == [output_item]
    assert provider.requests[1]["previous_response_delta_mode"] == "delta"
    assert len(provider.requests[1]["transport_attempts"]) == 2


def test_live_native_provider_exhausted_websocket_retry_is_retryable(tmp_path: Path) -> None:
    lane_input = _lane_input(tmp_path)
    provider = NativeCodexResponsesProvider(
        lane_input=lane_input,
        auth={"access_token": "x"},
    ...
    with patch(
        ...
        with pytest.raises(codex_api.CodexTransientTransportError):
            provider.next_response(
                {
                    "lane_attempt_id": "ws-native:task-native:implement_v2:native",
                    "turn_index": 1,
    ...
    assert provider.requests[0]["native_transport_failure_class"] == (
        "transient_provider_transport"
    )
    assert provider.requests[0]["retryable"] is True
    assert [attempt["status"] for attempt in provider.requests[0]["transport_attempts"]] == [
        "retryable_error",
        "retryable_error",
    ]
