...
class BriefTests(unittest.TestCase):
    ...
    def test_active_roadmap_focus_reads_current_active_work_m6(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with open(status, "w", encoding="utf-8") as handle:
                handle.write("# Roadmap Status\n\nActive work: **M7 Senses: Inbound Signals**.\n")

            self.assertEqual(
                active_roadmap_self_improve_focus(tmp),
                "Advance M7 Senses: Inbound Signals",
            )
    ...
    def test_next_move_coding_filter_skips_older_m6_paused_work_for_active_m7_focus(self):
        state = default_state()
        add_task(state, task_id=7, title="M6.7 paused debug target", kind="coding")
        state["work_sessions"].append(
            {
                ...
                "status": "active",
                "title": "M6.7 paused debug target",
                "goal": "Closed milestone work should not override the active M7 gate.",
                "created_at": "then",
                "updated_at": "now",
                "stop_requested_at": "2026-04-21T15:00:00Z",
                "stop_reason": "paused old target",
        ...
        with patch(
            ...
            return_value="Advance M7 Senses: Inbound Signals",
        ):
            self.assertEqual(
                next_move(state, kind="coding"),
                "start a native self-improvement session with `./mew self-improve --start-session --focus 'Advance M7 Senses: Inbound Signals'`",
            )
