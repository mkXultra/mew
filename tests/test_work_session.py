...
class WorkSessionTests(unittest.TestCase):
    ...
    def test_work_session_cells_capture_model_command_diff_and_approval(self):
        ...
        self.assertEqual(cells[2]["diff_stats"], {"added": 1, "removed": 1})
        self.assertTrue(cells[2]["dry_run"])
        ...
        self.assertIn("diff_stats: +1 -1", text)
    ...
    def test_work_session_tool_without_task_prints_one_shot_hint(self):
        old_cwd = os.getcwd()
        with tempfile.TemporaryDirectory() as tmp:
            os.chdir(tmp)
            try:
    ...
    def test_work_ai_pairing_steer_survives_noop_test_edit(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                Path("tests").mkdir()
                Path("src/mew/pairing.py").write_text("VALUE = 'old'\n", encoding="utf-8")
                Path("tests/test_pairing.py").write_text("def test_existing():\n    assert True\n", encoding="utf-8")
                with state_lock():
                    state = load_state()
                    add_coding_task(state)
                    save_state(state)
                ...
                model_outputs = [
                    ...
                    {
                        "summary": "make a noop paired test edit",
                        "action": {
                            "type": "edit_file",
                            "path": "tests/test_pairing.py",
                            "old": "assert True",
                    ...
                    {
                        "summary": "add the real paired test",
                        "action": {
                            "type": "write_file",
                            "path": "tests/test_pairing_added.py",
                            "content": "def test_pairing_added():\n    assert True\n",
                            "create": True,
                            "dry_run": False,
                        },
                    },
    ...
    def test_work_accept_edits_escalates_self_improve_governance_edit(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                model_output = {
                    "summary": "preview roadmap status edit",
                    "action": {
                        "type": "edit_file",
                        "path": "ROADMAP_STATUS.md",
                        "old": "old status",
                ...
                output = stdout.getvalue()
                self.assertIn("inline_approval=safety_blocked", output)
                self.assertIn("M5 safety boundary blocks automatic approval", output)
                self.assertIn("stop=pending_approval", output)
                self.assertEqual(Path("ROADMAP_STATUS.md").read_text(encoding="utf-8"), "old status\n")
                session = load_state()["work_sessions"][0]
                self.assertEqual(len(session["tool_calls"]), 1)
    ...
    def test_work_think_prompt_guides_independent_reads_to_batch(self):
        ...
        self.assertIn("Do not invent test-only assertions for behavior you have not observed", prompt)
        ...
        self.assertIn('"plan_items": ["short remaining steps when more than one concrete step remains (max 3)"]', prompt)
    ...
    def test_work_model_uses_suggested_verify_command_when_configured_verifier_missing(self):
        from mew.work_loop import normalize_work_model_action

        suggested = "uv run python -m unittest tests.test_commands"
        action = normalize_work_model_action(
            {"summary": "verify", "action": {"type": "run_tests"}},
            verify_command="",
            suggested_verify_command=suggested,
        )
        self.assertEqual(action["type"], "run_tests")
        self.assertEqual(action["command"], suggested)
    ...
    def test_work_model_context_over_budget_compacts_recent_window(self):
        ...
        self.assertEqual(work_context["recent_read_file_windows"][0]["tool_call_id"], 60)
        self.assertTrue(work_context["recent_read_file_windows"][0]["context_truncated"])
        self.assertIn("recent_read_file_windows", work_context["context_compaction"]["note"])
        self.assertLessEqual(len(json.dumps(work_context, ensure_ascii=False)), WORK_CONTEXT_BUDGET)

    def test_work_model_context_over_budget_keeps_exact_recent_line_window(self):
        from mew.work_loop import WORK_CONTEXT_BUDGET, build_work_model_context

        huge_text = "x" * 50000
        exact_text = "build_work_think_prompt exact window\n" * 40
        tool_calls = [
            {
                "id": index + 1,
                "tool": "read_file",
                "status": "completed",
                "parameters": {"path": f"file{index}.py"},
                "result": {"path": f"file{index}.py", "text": huge_text, "offset": 0},
                "summary": f"read file{index}",
            }
        ...
        tool_calls.append(
            {
                ...
                "tool": "read_file",
                "status": "completed",
                "parameters": {"path": "src/mew/work_loop.py", "line_start": 760, "line_count": 40},
                "result": {
                    "path": "src/mew/work_loop.py",
                    "line_start": 760,
                    "line_end": 799,
                ...
                "summary": "read exact prompt window",
            }
        )
        session = {
            "id": 1,
            "task_id": 1,
            "status": "active",
            "goal": "Keep exact recent line window.",
            "created_at": "then",
            "updated_at": "now",
            "tool_calls": tool_calls,
            "model_turns": [],
        ...
        task = {
            "id": 1,
            "title": "Budget",
            "description": "Keep exact recent line window.",
            "status": "todo",
            "kind": "coding",
        }
        ...
        self.assertEqual(work_context["recent_read_file_windows"][0]["line_end"], 799)
    ...
    def test_workbench_surfaces_work_session_reentry_guidance(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertNotIn("task_notes:", output)
                self.assertNotIn("[...2 older work-session finish notes omitted...]", output)
    ...
    def test_chat_continue_runs_active_work_session_live_step(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertIn("/work-session details", output)
                self.assertIn("/help work for diffs", output)
    ...
    def test_work_session_keeps_memory_for_applying_approval(self):
        from mew.work_session import build_work_session_resume
            ...
            "model_turns": [
                {
                    ...
                    "decision_plan": {
                        "working_memory": {
                            "hypothesis": "A write is being approved.",
        ...
        resume = build_work_session_resume(session, task={"id": 1, "title": "Approval applying"})
    ...
    def test_work_follow_status_task_arg_does_not_fall_back_to_unrelated_latest(self):
        from mew.timeutil import now_iso
