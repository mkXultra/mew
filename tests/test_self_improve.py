...
class SelfImproveTests(unittest.TestCase):
    ...
    def test_cli_self_improve_help_describes_native_work_flow(self):
        ...
        self.assertIn("prints concrete continue/follow/status/resume controls", output)
        self.assertIn("mew self-improve --start-session --json --focus", output)
        self.assertIn("returns controls.continue, controls.follow, controls.status, controls.resume", output)
