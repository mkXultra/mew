...
def build_work_think_prompt(context):
    return (
        ...
        "If a task names an exact external ground-truth command, tool, binary, or required flags, run that exact command shape or a verifier that invokes it before finish; surrogate libraries, approximations, or nearby tools are not enough unless the task explicitly allows them. Cite the completed run_command or run_tests tool id in acceptance_checks evidence. "
