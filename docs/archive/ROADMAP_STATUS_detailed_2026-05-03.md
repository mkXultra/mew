...
| 6.24 Software/Coding Terminal-Bench Parity Campaign | `in_progress` | Scope changed on 2026-05-03 to the 25 Terminal-Bench 2.0 `software-engineering,coding` tasks; scoped rebaseline pending. `compile-compcert` remains historical build-orchestration evidence, not the active M6.24 close gate. |
...
- User decision on 2026-05-03: narrow M6.24 close scope to the 25
  Terminal-Bench 2.0 tasks returned by `software-engineering,coding`; prior
  all-registry and `compile-compcert` evidence remains historical unless a
  later build-orchestration milestone reopens it.
...
- Current next action: build the scoped 25-task cohort rebaseline against frozen
  Codex targets and measured mew artifacts, then select the first below-target
  in-scope gap.
...
Historical selected gap class:
  ...
  `docs/DESIGN_2026-05-01_M6_24_LONG_BUILD_SUBSTRATE.md`. Phase 0 is complete:
...
Historical selected gap class:
  ...
  verifier passed VM execution, frame existence, and frame similarity. The
  five-trial proof is recorded in
  `docs/M6_24_MAKE_MIPS_PROOF_5_2026-04-29.md`: score improved to `1/5` with
  no runner errors, but stayed below the frozen Codex target `3/5`. Repeated
  failures now include 320x200 frames accepted by self-checks where the
  external verifier expected 640x400. The v0.4 generic repair adds a
  runtime-visual-artifact quality finish guard plus THINK guidance: format,
  existence, nonzero pixels, or self-consistent dimensions are not enough when
  the task asks for correct/expected rendered output. The v0.4 speed proof is
  recorded in `docs/M6_24_VISUAL_QUALITY_SPEED_RERUN_2026-04-29.md`:
...
- M6.24 `compile-compcert` v0/v0.1 follow-up work is recorded in
  ...
  implemented and passed speed_1 at `1/1`. Next action is a
  resource-normalized five-trial `compile-compcert` proof with `-k 5 -n 1` and
  `auth.plus.json`.
- Closed M6.14 follow-on episode:
