...
for omitted/truncated/empty command output, so compact live transcript output
still exposes output size/truncation/live-omission state, stable refs, and
`read_command_output(command_run_id=...)` readback when full output is not
already visible, without adding provider-visible action hints.
...
- surface factual command output access metadata for omitted/truncated/empty
  terminal output: bytes, truncation/live-omission state, stable output ref,
  and readback command id when readback is actually needed;
...
- command/test fixtures expose factual output metadata when full terminal
  output is omitted, truncated, or empty, without repeating readback metadata
  for small output that is already visible;
