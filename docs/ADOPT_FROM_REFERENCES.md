...
- 2026-04-19 実装メモ：5.12 の MVP は `mew memory --add --type ... --scope ...` と file-backed recall として入り始めた。`work_session.resume.active_memory` として resident THINK prompt にも渡るようになったので、次は実 API dogfood で判断に効くか見る。
