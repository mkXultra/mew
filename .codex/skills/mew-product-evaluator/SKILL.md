...
- 5.12 has a file-backed typed/scoped memory MVP, and native work resume now injects an `active_memory` bundle into the resident THINK prompt. The next product question is whether this actually changes resident behavior in real API dogfood, not whether more storage surfaces exist.
