# mew-ghost SP12 shell scaffold

`mew-ghost` is an isolated side project for a permission-safe macOS presence shell. SP12 is fixture-driven: it does not import core mew code, read live `.mew` state, call live macOS APIs, capture the screen, monitor hidden activity, use the network, or package a native app.
...
- `tests/test_mew_ghost.py`: focused tests for fixture rendering, probe contract, CLI output, isolation, and dry-run launch intents.
...
On non-macOS platforms the probe returns `unavailable` without calling the provider. On macOS, callers may inject a provider; `PermissionError` is converted to `permission_denied` instead of prompting or retrying. With no provider, the live probe is explicitly deferred.
