...
`feal-differential-cryptanalysis`, `feal-linear-cryptanalysis`, `fix-git`, and
`hf-model-inference` v2 speed_1 runs passed with exact replay and
terminal-bench replay dogfood. The
...
```text
M6.24 -> implement_v2 scoped rebaseline -> next pending task is kv-store-grpc -> selected_lane=implement_v2 speed_1 per scoped software/coding task -> if pass, record v2 evidence and continue queue -> if miss/harness-invalid/missing-replay/structural-gap, stop unrelated measurement, reproduce through replay/dogfood/emulator, repair, then rerun same shape
```
