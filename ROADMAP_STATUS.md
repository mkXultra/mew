...
- Using mew for one focused coding task feels close to Claude Code / Codex CLI:
  ...
  interrupted fresh CLI resume. Follow-up dogfood task `#270` made the prompt
  contract clearer by emitting an explicit `mew_session_argument` alias while
...
- The synthetic aged reentry layer now covers week-scale age. `day-reentry`
  synthesizes a work session older than 7 days before exercising normal
  `mew focus`, `mew work --session --resume`, and `mew activity` surfaces.
  `/tmp/mew-aged-reentry-7day-20260420` passed with
  `observed_inactive_hours=191.4`, preserved unresolved verifier risk, restored
  working memory and live README.md world-state, and kept old work events
  visible for audit. The durable summary is in
  `docs/M3_AGED_REENTRY_7DAY_2026-04-20.md`.
- High-dilation resident-loop dogfood exposed and fixed a day-scale passive
  memory-bloat issue: recurring passive `self_review` actions were not part of
  repeated-wait thought compaction and appended duplicate deep-memory decisions
  every logical few hours. Passive wait compaction now admits self-review-only
  waits, and duplicate self-review decisions are skipped. The targeted
  high-dilation smoke at `/tmp/mew-dilation-self-review-compact-20260420`
  passed with `time_dilation=3600.0`, `passive_span_seconds=14579.0`, thought
...
- Proof isolation now has a local Docker foundation. `Dockerfile.proof`,
  `.dockerignore`, `scripts/run_proof_docker.sh`, and
  `scripts/collect_proof_docker.sh` can build a proof image, run resident-loop
  cadence proof in a detached container with per-container mounted
  workspace/artifact directories, then collect `docker inspect`, logs, and a
  short summary after completion. This is the low-cost first step from
...
- The first isolated 10-day virtual-time resident proof is complete. A 90-second
  Docker run with `MEW_PROOF_TIME_DILATION=10080` first failed by exposing
...
- The M3 proof shape now extends beyond README-only tasks.
  `m3-source-reentry` creates a Python source file plus unittest, preserves a
  pending source edit and failed `run_tests` result through resume/follow
  surfaces, then proves the resident can approve the edit and reach passing
  unittest. Local and Docker runs both passed; Docker proof
  `mew-proof-m3-source-reentry-20260420-1058` recorded `continuity_score=9/9`,
...
- Task-local resume and scoped reentry views are proven across a deterministic
  week-scale synthetic interruption/resume cycle, but not yet across real
  multi-day resident runtime operation.
...
- Failed `run_command` work tools now enter the recovery plan as
...
- Live write/edit approval prompts now create a durable `work_approval`
  question before reading stdin. If the prompt is interrupted or receives EOF,
  the approval request remains visible in questions, outbox, attention, and the
  work-session pending-approval resume surface; successful approval or
  rejection answers the linked question, while failed/interrupted apply attempts
  leave it open for review. This adopts the M4-relevant Durable Elicitation
  idea from `docs/ADOPT_FROM_REFERENCES.md` without making approvals
  autonomous. Durable summary:
...
- Task `#308` produced a third reviewed no-rescue candidate. The resident
  independently inspected rescue `loop_credit_status` coverage, determined the
  source behavior from task `#307` was already correct, and proposed a focused
